...
| Refetch/retry loop in the live build (e.g. an expired-token request retried with no backoff) | **Guarded in-code:** the retry policy (`src/lib/queryRetry.ts`) never retries a 4xx/5xx/timeout/abort/server-coded error and uses capped exponential backoff, so a failing request can't amplify into a flood; and a client-side circuit breaker (`src/lib/data/requestCircuitBreaker.ts`) sheds a failing loop on **every bounded read** — the `feed_items`/`feed_unread_counts` RPCs, the `item_state` hydration GET, and ordinary table reads (`subscriptions`, `user_settings`, `folders`, …) — once its requests rack up consecutive failures (failure-based, so a legitimate bulk burst like an offline warmup won't trip it). What the path decides is how much an outcome COUNTS: on a cacheable read a 5xx, a thrown error and an auth/permission 4xx are failure evidence, while a `200` and a `400`/`404` carrying a PostgREST missing-table/column/function code (`PGRST202`/`PGRST204`/`PGRST205`/`42P01`/`42703` — a capability probe against a backend that is merely behind the client) are not — the SW's `NetworkFirst` can answer a cacheable read from cache, so a `200` may be one the backend never saw. The service worker stamps every REST read it handles with `x-readmo-sw-source: network|cache-error|cache-timeout` so those can be told apart at all: `cache-error` means the request failed and the cache answered instead (failure evidence — without it a warm cache masks an outage as a string of `200`s), `cache-timeout` means the cache answered because the read outran the worker's 6s window while still in flight (a slow backend is not a failing one, so it counts as nothing), `network` is the backend itself answering (authoritative, so an ordinary table read can confirm a recovery), and **no stamp** means an older service worker is still installed, so the response stays inconclusive. Being **offline** never counts: shedding happens before the service worker, so tripping the breaker with no network would cost the reader their offline cache for nothing. Recovery on that weaker evidence reopens the circuit with admission serialized to one request at a time until a real backend success confirms it, so an operator seeing reads trickle rather than resume in parallel is looking at a breaker that has recovered but not yet been confirmed. A *succeeding* high-rate loop is shed at the gateway instead. If a loop still gets through, arm `MIN_CLIENT_BUILD` and rate-limit at the gateway (below). |
