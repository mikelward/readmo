...
export function ItemList({
  ...
  const emptyMoreSections = useMemo(() => {
    ...
    const feedMeta = new Map<FeedId, { title: string; faviconUrl?: string | null }>();
    for (const fi of items) {
      if (!feedMeta.has(fi.item.feedId)) {
        feedMeta.set(fi.item.feedId, {
          title: fi.feed.title,
    ...
    for (const feedId of feedsWithMore) {
      ...
      const meta = feedMeta.get(feedId);
      if (meta === undefined) continue; // feed dropped out of items entirely
      out.push({ feedId, title: meta.title, faviconUrl: meta.faviconUrl });
    }
