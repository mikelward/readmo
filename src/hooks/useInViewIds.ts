...
export function useInViewIds(opts: Options = {}): {
  ...
  // Mirrors the observer's geometry exactly: its root is the viewport shrunk by
  // the sticky-chrome insets *after the same `insetWithSlack` tolerance is
  // applied* (that's what shapes the observer's `rootMargin`), and a row counts
  // as fully visible at the same FULLY_VISIBLE_RATIO. A full-width row is always
  // fully within the root horizontally, so its area ratio reduces to the
  // vertical-overlap fraction — exactly what the observer reports. Reading a rect
  // forces layout, so this sees the post-reflow positions even when called from a
  // rAF right after the removal.
  const resync = useCallback(() => {
    if (typeof window === 'undefined') return;
    const rootTop = insetWithSlack(topInset);
    const rootBottom = window.innerHeight - insetWithSlack(bottomInset);
    setInViewIds((prev) => {
      let changed = false;
      const next = new Set(prev);
