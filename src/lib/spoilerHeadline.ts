...
//     (useReadingPrefs).
// Only when BOTH hold AND a non-empty rewrite exists do we swap the headline;
// otherwise the original title is shown untouched (the safe, reversible default).
...
export interface DisplayTitle {
   ...
   * original — the signal to render the "de-spoilered" marker. */
  rewritten: boolean;
...
/** Pick the headline to display for an item, honoring the allowlist gate and the
 * per-user setting. Falls back to the original title whenever a rewrite can't or
 * shouldn't be shown (off-allowlist, setting off, no rewrite cached, or a
 * blank/whitespace rewrite), so a missing/empty column never blanks a row. */
export function displayTitle(
  item: Pick<Item, 'title' | 'spoilerFreeTitle'>,
  { hideSpoilers, allowed }: SpoilerHeadlineOptions,
): DisplayTitle {
  const original = item.title;
  const rewrite = item.spoilerFreeTitle?.trim();
  if (hideSpoilers && allowed && rewrite) {
