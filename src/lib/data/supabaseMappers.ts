...
export function mapItem(row: ItemRow): Item {
  // A cached full body is usable only when stamped with the current extractor
  // version; otherwise it's stale (older version, or legacy NULL).
  const fullFresh =
  ...
  return {
    ...
    // fullContentStale records that a body WAS present but dropped, so the reader
    // can re-extract even when the feed body isn't truncated.
    fullContentHtml: fullFresh ? row.full_content_html! : null,
    fullContentStale: !!row.full_content_html && !fullFresh,
