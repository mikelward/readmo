...
check "markdown under supabase/ is a build input" 0 "docs_only=false" FILES="supabase/functions/notes.md"
...
export MODE=gate
ALLOK="build=success edge=success"
ALLSKIP="build=skipped edge=skipped"
...
check "half-skipped is not justified"     1 "not all green" FILES="src/a.ts" CLASSIFY=success RESULTS="build=skipped edge=success"
check "red job fails the gate"            1 "not all green" FILES="src/a.ts" CLASSIFY=success RESULTS="build=failure edge=success"
check "cancelled job fails the gate"      1 "not all green" FILES="src/a.ts" CLASSIFY=success RESULTS="build=cancelled edge=cancelled"
