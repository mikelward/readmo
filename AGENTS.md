...
| **Google Gemini** (`gemini-2.5-flash-lite`) | Generates the short AI article summary (a few sentences) shown at the top of the reader for any article an allowlisted user opens (the `summary` Edge Function; a pin pre-warms it ahead of the open). Configured via the `GOOGLE_API_KEY` Supabase secret; the function reports `unavailable` and the reader shows no card if unset. Summarizes the article text fetched via **Jina** (markdown), falling back to the stored sanitized body if Jina is unavailable. | Flash-Lite ≈ $0.10 / 1M input tokens, $0.40 / 1M output. A summary is a few-K-token input + a short-paragraph output, generated **once per article** (cached on the shared item), so even though it now runs for every article an allowlisted user opens (not just pinned), miss volume is bounded by *distinct articles read* across the family set — **effectively $0**, well under the free credit. (Jina's own cost is in its row.) | Standard Google AI Studio quotas (generous; far above family read volume — only a cache miss calls out). | One Jina fetch + one Gemini call (~2–7 s total) on a cache miss — the first open (or pin pre-warm) of an article; cache-instant thereafter. On the read path now (first open), but only on a miss. | Gemini down / non-2xx / timeout → `unreachable` (retryable), reader shows no card; key unset → `unavailable`. All soft — the article and reading mode are unaffected. |
