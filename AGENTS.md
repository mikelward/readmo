...
| **HN Algolia API** (`hn.algolia.com`) | Reader comments icon: resolves an article URL → its Hacker News discussion (story id), client-side, so the reader can link out to the thread on newshacker (`src/lib/hnDiscussion.ts`). Fired per opened article on a **public** feed while online. | **Negligible** — free, keyless public API (the same index newshacker searches); no quota signup-scale traffic approaches. | Generous public limits; one request per reader open, cached forever per URL. | One `fetch` off the critical path: the article renders immediately; the icon just appears (or not) when the lookup lands. | On offline / non-2xx / malformed / no-match, `findHnDiscussion` returns `null` → no icon. Never a dead control; no change from today's no-comments reader. |
