...
- **Read the Codex verdict, don't infer it.** It reacts to the PR body
  ...
  finding* below); an acknowledgement is not an answer. Nothing from Codex
  since the push, five minutes on, or a clean review that left no reaction,
  leaves the `codex` status pending — comment `@codex review`, once; escalate
  rather than poking again if the status is still pending five minutes later.
